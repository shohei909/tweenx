...
class TweenX extends CommandX {
	...
	private static function get_tweens():Iterable<TweenX> {
		return _tweens;
	}
	...
	private static var _rules:Array<RuleX<Dynamic, Dynamic>> = [
		BoolRuleX,
		ArrayRuleX,
		TimelineRuleX,
		ArgbRuleX,
	...
	private static function get_rules():Iterable<RuleX<Dynamic, Dynamic>> {
		return _rules;
	}
	...
	public static var updateMode(default,
		#if haxe3 set #else set_updateMode #end):UpdateModeX = #if (flash) UpdateModeX.FRAME; #else UpdateModeX.TIME(60); #end
	...
	public static function from<T>(target:T, ?_to:Dynamic, ?delay:Float, ?repeat:Int, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos) {
		...
		if (!isIterable(target))
			return new StandardTweenX<T>(FROM_TO(target, {}, _to), 0, EaseX.linear, delay, repeat, false, false, interval, autoPlay, posInfos);
		else
			return new StandardTweenX<T>(ARRAY(untyped target, [{}], [_to]), 0, EaseX.linear, delay, repeat, false, false, interval, autoPlay, posInfos);
	}

	public static function to<T>(target:T, ?_to:Dynamic, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool,
			?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos) {
		if (_to == null)
			_to = {};
		if (!isIterable(target))
			return new StandardTweenX<T>(FROM_TO(target, {}, _to), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay, posInfos);
		else
			return new StandardTweenX<T>(ARRAY(untyped target, [{}], [_to]), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay, posInfos);
	}

	public static function tweenFunc(func:Dynamic, _from:Iterable<Dynamic>, _to:Iterable<Dynamic>, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int,
			?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos) {
	...
	public static function tweenFunc1(func:Float->Void, from1:Float, to1:Float, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool,
			?zigzag:Bool, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos) {
		return new TweenX(FUNC(func, [from1], [to1]), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay, posInfos);
	}

	public static function tweenFunc2(func:Float->Float->Void, from1:Float, from2:Float, to1:Float, to2:Float, ?time:Float, ?ease:Float->Float, ?delay:Float,
			?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos) {
		return new TweenX(FUNC(func, [from1, from2], [to1, to2]), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay, posInfos);
	}

	public static function tweenFunc3(func:Float->Float->Float->Void, from1:Float, from2:Float, from3:Float, to1:Float, to2:Float, to3:Float, ?time:Float,
			?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos) {
		return new TweenX(FUNC(func, [from1, from2, from3], [to1, to2, to3]), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay, posInfos);
	}

	public static function tweenFunc4(func:Float->Float->Float->Float->Void, from1:Float, from2:Float, from3:Float, from4:Float, to1:Float, to2:Float,
			to3:Float, to4:Float, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool,
			?posInfos:PosInfos) {
		return new TweenX(FUNC(func, [from1, from2, from3, from4], [to1, to2, to3, to4]), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay,
			posInfos);
	}
	...
	public static function wait(?delay:Float = 0.1, ?posInfos:PosInfos) {
		return new CommandX(WAIT(delay), posInfos);
	}
	...
	private static inline function _lock(o:TweenX) {
		if (o._inited)
			throw o.error("Can't serialize initialized TweenCommandX");
		o._autoPlay = false;
	}
	...
	private static inline function hashObject(o:Dynamic) {
		...
		#elseif flash9
		untyped if (dictionary[o] != null)
			return untyped dictionary[o];
		else {
			_objCounter = 1 + (_objCounter % 33029);
	...
	private static inline function setField(o:Dynamic, key:String, value:Dynamic) {
		...
		#elseif (js)
		var tmp;
		untyped if (o.__properties__ && (tmp = o.__properties__["set_" + key]))
			o[tmp](value);
		else
			o[key] = value;
		#else
	...
	private static function isIterable(d:Dynamic) {
		return (d != null
			&& (Std.is(d, Array) || Reflect.hasField(d, "iterator") && Reflect.isFunction(d.iterator) && d.iterator() != null));
	}
	...
	private static var EVENT_ARRAY = [
		"play", "delay", "head", "update", "foot", "interval", "repeat", "rest", "finish", "stop"
	];
	 ...
	 * コンストラクタ
	 */
	function new(type:TweenTypeX, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool,
			?posInfos:PosInfos) {
		...
		switch (type) {
			...
			default:
				_ease = (_easeIsDefault = ease == null) ? defaultEase : ease;
		}
	...
	private function set_timeScale(value:Float) {
		return if (_parent != null) throw error("Can't change timeScale of serialized object directly"); else timeScale = value;
	}
	...
	public function play(#if (tweenx_debug) ?posInfo:PosInfos #end) {
		if (_parent != null)
			throw error("Can't play serialized object directly");
		if (playing)
			return this;
		if (!_inited)
			_init();
	...
	public function stop() {
		if (_parent != null)
			throw error("Can't stop serialized object directly");
		_stop();
		return this;
	}
	...
	public function goto(time:Float = 0, andPlay:Bool = false #if (tweenx_debug), ?posInfo:PosInfos #end) {
		if (_parent != null)
			throw error("Can't move serialized object directly");
		if (!_inited) {
			_init();
		}
		var t = time;
		if (t < 0)
			t = 0;
		else if (t > _totalTime)
			t = _totalTime;
		_update(t - currentTime);
		if (andPlay) {
			play(#if (tweenx_debug) posInfo #end);
		}
		return this;
	}
	...
	private function _init() {
		...
		if (_groupDefaults) {
			...
			if (_delayIsDefault)
				_delay = defaultDelay;
			if (_intervalIsDefault)
				_interval = defaultInterval;
			if (_repeatIsDefault)
				_repeat = defaultRepeat;
			if (_yoyoIsDefault)
				_yoyo = defaultYoyo;
			if (_zigzagIsDefault)
				_zigzag = defaultZigZag;
			if (_autoPlayIsDefault)
				_autoPlay = defaultAutoPlay;
		}

		if (_repeat == 0)
			_repeat = #if (neko && !haxe3) 0x3FFFFFFE #else 0x7FFFFFFE #end;
		if (_time < _MIN)
			_time = _MIN;
	...
	private function _apply(p:Float, repeatNum:Int) {
		...
		switch (_type) {
			case FROM_TO(target, _from, _to):
				...
				if (_fastMode) {
					for (key in _toKeys) {
						setField(target, key, _fastCalc(field(_from, key), field(_to, key), t, t2));
					}
				} else {
					for (key in _toKeys) {
						setField(target, key, _calc(field(_from, key), field(_to, key), t, t2));
					}
				}
			...
			case FUNC(func, _from, _to):
				var t2 = 1 - t;
				var arr = [];
				for (i in 0..._to.length)
					arr[i] = _calc(_from[i], _to[i], t, t2);
	...
	public function time(value:Float) {
		if (value < 0)
			throw error("Can't be negative value");
		checkInited();
	...
	private inline function _addEventListener(type:String, listener:Dynamic) {
		...
		if (i < 0)
			return;
		var arr = if (_eventListeners[i] == null) _eventListeners[i] = []; else _eventListeners[i];
		if (!arr.has(listener))
			arr.push(listener);
	}
	...
	private function checkInited():Void {
		if (_inited)
			throw error("Can't change params after initialization");
	}
