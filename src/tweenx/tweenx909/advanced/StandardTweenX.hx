...
class StandardTweenX<T> extends TweenX {
	...
	public function new(type:TweenTypeX, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float,
			?autoPlay:Bool, ?posInfos:PosInfos) {
		switch (type) {
		...
		if (TweenX._initLog[id] == null) {
			...
			for (log in TweenX._initLog[id]) {
				if (log.target == target) {
					for (key in TweenX.fields(data)) {
						TweenX.setField(log.data, key, TweenX.field(data, key));
					}
					flag = true;
					break;
				}
	...
	private inline function _initFrom(target, _from, _to) {
		...
		for (key0 in TweenX.fields(_to)) {
			if (!Std.isOfType(TweenX.field(_to, key0), Float))
				_fastMode = false;
