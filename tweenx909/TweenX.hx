...
import tweenx909.rule.AhsvX;
import tweenx909.rule.ArgbX;
...
class TweenX extends CommandX {
	...
	static public var tweens( #if haxe3 get #else get_tweens #end, never ):Iterable<TweenX>;
	 ...
	 * 基本パラメータデフォルト値
	 */
	static public var DEFAULT_EASE(default,null):Float->Float = EaseX.linear;
	static public inline 	var DEFAULT_TIME:Float		= 0.3;
	static public inline 	var DEFAULT_DELAY:Float		= 0;
	static public inline 	var DEFAULT_REPEAT:Int		= 1;
	static public inline 	var DEFAULT_INTERVAL:Int	= 0;
	static public inline 	var DEFAULT_YOYO:Bool		= false;
	static public inline 	var DEFAULT_ZIGZAG:Bool		= false;
	static public inline 	var DEFAULT_AUTO_PLAY:Bool	= true;
	static private inline 	var DEFAULT_AUTO_FROM:Bool	= true;
	
	static public var 	defaultEase:Float->Float	= TweenX.DEFAULT_EASE;
	static public var 	defaultTime:Float 			= TweenX.DEFAULT_TIME;
	static public var 	defaultDelay:Float			= TweenX.DEFAULT_DELAY;
	static public var 	defaultInterval:Float		= TweenX.DEFAULT_INTERVAL;
	static public var 	defaultRepeat:Int			= TweenX.DEFAULT_REPEAT;
	static public var 	defaultYoyo:Bool 			= TweenX.DEFAULT_YOYO;
	static public var 	defaultZigZag:Bool 			= TweenX.DEFAULT_ZIGZAG;
	static public var 	defaultAutoPlay:Bool		= TweenX.DEFAULT_AUTO_PLAY;
	static private var 	defaultAutoFrom:Bool		= TweenX.DEFAULT_AUTO_FROM;
	
	static private	var _rules:Array<RuleX<Dynamic,Dynamic>> 			= [BoolRuleX, ArrayRuleX, TimelineX, RgbX, HsvX, ArgbX, AhsvX, QuakeX];
	static public 	var rules( #if haxe3 get #else get_rules #end, never ):Iterable<RuleX<Dynamic,Dynamic>>;
	...
	static public function dumpDefaults() {
		return new DefaultsX().dump();
	}
	static public function setDefaults( defaults:DefaultsX ) {
		defaults.apply();
	}
	static public function initDefaults() {
		new DefaultsX().apply();
	}
	
	static public 	var updateMode( default, #if haxe3 set #else set_updateMode #end ):UpdateModeX	
	...
	static public function manualUpdate( time:Float #if(tweenx_debug) ,?posInfo:PosInfos #end  ) {
		initTweens();
	 ...
	 * 変則ルールの管理
	 */
	static public function addRule( rule:RuleX <Dynamic,Dynamic> ) {
	...
	static public function tweenFunc2( func:Float->Float->Void, from1:Float, from2:Float, to1:Float, to2:Float, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos ) {
		return new TweenX( FUNC( func, [from1,from2], [to1,to2] ), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay, posInfos );
	}
	static public function tweenFunc3( func:Float->Float->Float->Void, from1:Float, from2:Float, from3:Float, to1:Float, to2:Float, to3:Float, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos ) {
		return new TweenX( FUNC( func, [from1,from2,from3], [to1,to2,to3] ), time, ease, delay, repeat, yoyo, zigzag, interval, autoPlay, posInfos );
	}
	static public function tweenFunc4( func:Float->Float->Float->Float->Void, from1:Float, from2:Float, from3:Float, from4:Float, to1:Float, to2:Float, to3:Float, to4:Float, ?time:Float, ?ease:Float->Float, ?delay:Float, ?repeat:Int, ?yoyo:Bool, ?zigzag:Bool, ?interval:Float, ?autoPlay:Bool, ?posInfos:PosInfos ) {
	...
	static public function func( func:Void->Void, ?delay:Float, ?repeat:Int, ?interval:Float, ?posInfos:PosInfos ) {
	 ...
	 * 一括制御
	 */
	static public function playAll( tweens:Iterable<TweenX> #if(tweenx_debug) ,?posInfo:PosInfos #end ) {
		for ( t in tweens ) 
			switch( t.command ) {
				case WAIT( _ ):
				case TWEEN( o ):
	...
	static public function stopAll( tweens:Iterable<TweenX> ){
		for ( t in tweens )
			switch( t.command ) {
				case WAIT( _ ):
				case TWEEN( o ):
	...
	static public function gotoAll( tweens:Iterable<TweenX>, time:Float = 0, andPlay:Bool = false #if(tweenx_debug) ,?posInfo:PosInfos #end ){
		for ( t in tweens ) 
			switch( t.command ) {
				case WAIT( _ ):
				case TWEEN( o ):
	...
	static public function updateAll( tweens:Iterable<TweenX>, time:Float #if(tweenx_debug) ,?posInfo:PosInfos #end  ) {
		for ( t in tweens ) 
			switch( t.command ) {
				case WAIT( _ ):
				case TWEEN( o ):
	 ...
	 * シリアライズ
	 */
	static public function serial( tweens:Iterable<CommandX>, ?defaults:DefaultsX, ?posInfos:PosInfos ) {
		return _group( tweens, SERIAL, defaults, posInfos );
	}
	static public function lag( tweens:Iterable<CommandX>, ?delay:Float = 0.1, ?defaults:DefaultsX, ?posInfos:PosInfos ) {
		return _group( tweens, LAG( delay ), defaults, posInfos );
	}
	static public function parallel( tweens:Iterable<CommandX>, ?defaults:DefaultsX, ?posInfos:PosInfos ) { 
		return _group( tweens, LAG( 0 ), defaults, posInfos );
	}
	static public function wait( ?delay:Float = 0.1, ?posInfos:PosInfos ) { return new CommandX( WAIT(delay), posInfos ); }
	...
	static private inline var _DELAY 	= Type.enumIndex( EventX.DELAY );
	static private inline var _FINISH 	= Type.enumIndex( EventX.FINISH );
	static private inline var _FOOT 	= Type.enumIndex( EventX.FOOT );
	static private inline var _HEAD 	= Type.enumIndex( EventX.HEAD );
	static private inline var _INTERVAL = Type.enumIndex( EventX.INTERVAL );
	static private inline var _PLAY 	= Type.enumIndex( EventX.PLAY );
	static private inline var _REPEAT 	= Type.enumIndex( EventX.REPEAT );
	static private inline var _REST 	= Type.enumIndex( EventX.REST );
	...
	static public var idCounter:Int = 0;
	...
	public function addEventListener( type:EventX, listener:TweenX->Void ) {
		_addEventListener( type, listener );
		return this;
	}
	public function removeEventListener( type:EventX, listener:TweenX->Void ) {
		_removeEventListener( type, listener );
		return this;
	}
	private inline function _addEventListener( type:EventX, listener:Dynamic ) {
		var i = Type.enumIndex( type );
		var arr = 	if ( _eventListeners[i] == null ) 	_eventListeners[i] = [];
					else								_eventListeners[i];
		if(! arr.has( listener ) ) arr.push( listener );
	}
	private inline function _removeEventListener( type:EventX, listener:Dynamic ) {
		var i = Type.enumIndex( type );
	...
	private inline function dispatch( num:Int ) {
		#if (tweenx_debug)
		if( num){
			var action = ["PLAY","DELAY","HEAD","UPDATE","FOOT","INTERVAL","REPEAT","REST","FINISH","STOP"][num];
			var p = definedPosInfos;
