...
{% include sample.asciidoc num="312" id="CustomEasing" %}
...
モーションの詰まり方が、もとの``quintOut``とは別のものになっているのがわかると思います。
...
サンプルは、``(x, y)=(210, 60)``を極座標の中心として、そこに``(x, y)=(0, 0)``の位置から回転しながら近づいていくモーションです。
...
{% include sample.asciidoc num="317" id="Color" %}
...
{% include sample.asciidoc num="318" id="Bitmap" width="151" %}
